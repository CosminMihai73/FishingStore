...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="css/pagina-obiect.css">
...
<?php
// Verificați dacă există un parametru "id" în URL
if (isset($_GET['id'])) {
    // Obțineți valoarea parametrului "id"
    $id = $_GET['id'];

    // Simulați conexiunea la o bază de date (aceasta trebuie să fie adaptată la baza de date reală)
    ...
    // Interogați baza de date pentru a obține informații despre obiectul cu ID-ul dat
    $sql = "SELECT * FROM articole WHERE id = $id";
    $result = $conn->query($sql);

    // Verificați dacă s-au găsit rezultate
    if ($result->num_rows > 0) {
        // Afisati informatiile despre obiect
        while ($row = $result->fetch_assoc()) {
            echo "<div class='pagina obiect'>";
            echo "<a href='pagina-obiect.php?id=" . $row["id"] . "'>";
            echo "<h3><a href='pagina-obiect.php?id=" . $row["id"] . "'>" . $row["title"] . "</a></h3>";
            echo "<img src='" . $row["image_url"] . "' alt='Imagine Articol'>";
            echo "<p>" . $row["Descrieremare"] . "</p>";
            echo "<p>Preț: RON " . $row["pret"] . "</p>";
            // Alte informații pot fi afișate aici în funcție de structura bazei de date
            // Add "Adauga in cos" button
            echo "<form method='post'>";
            echo "<input type='hidden' name='item_id' value='" . $row["id"] . "'>";
            echo "<input type='submit' name='add_to_cart' class='buton-obiect-add' value='Adauga in cos'>";
            echo "</form>";

            // Add "Favorite" button
            echo "<form method='post'>";
            echo "<input type='hidden' name='item_id' value='" . $row["id"] . "'>";
            echo "<input type='submit' name='add_to_favorite' class='button-favorite-add' value='Favorite'>";
            echo "</form>";
    ...
    // Dacă nu există parametrul "id" în URL, puteți trata această situație cum doriți
    echo "Lipsește parametrul 'id' în URL.";
}
?>
